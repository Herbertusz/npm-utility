 ...
 * @return {object} vektor nagysága és iránya (angle: felfelé mutató tengellyel bezárt szög)
 */
export const getVector = function(components: Coord): Vector {
    return {
        ...
        angle: Math.atan(components.x / components.y)
    };
};

/**
 * Koordináta meghatározása {length,angle} => {x,y}
 * @param vector - vektor nagysága és iránya (angle: felfelé mutató tengellyel bezárt szög)
 * @return {object} vektor x és y irányú komponense
 */
export const getCoord = function(vector: Vector): Coord {
    return {
        x: vector.length * Math.sin(vector.angle),
        y: vector.length * Math.cos(vector.angle)
    };
};
